...
class FormData:
    ...
    def _validate_data(self, data):
        invalid_fields = {}
        if not self._validate_fio(data["fio"]):
            invalid_fields["fio"] = data["fio"]
        if not self._validate_date(data["birthdate"]):
            invalid_fields["birthdate"] = data["birthdate"]
        if not self._validate_date(data["passport_issue_date"]):
            invalid_fields["passport_issue_date"] = data["passport_issue_date"]
        if not self._validate_gender(data["gender"]):
            invalid_fields["gender"] = data["gender"]
